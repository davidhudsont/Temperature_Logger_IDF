...
static void register_task_info_command(void);
...
static struct
{
    struct arg_lit *stack_usage;
    struct arg_lit *processor_usage;
    struct arg_end *end;
} task_info_args;
...
static esp_err_t print_real_time_stats(TickType_t xTicksToWait)
{
    ...
    esp_err_t ret;
    static const UBaseType_t ARRAY_SIZE_OFFSET = 5; //Increase this if print_real_time_stats returns ESP_ERR_INVALID_SIZE
    ...
    {
        ESP_LOGI("STATS", "end time = %d start time = %d", end_run_time, start_run_time);
    ...
    printf("| Task | Run Time | Percentage | Stack Left\n");
    ...
    for (int i = 0; i < start_array_size; i++)
    {
        ...
        if (k >= 0)
        {
            ...
            printf("| %s | %d | %d%% | %d\n", start_array[i].pcTaskName, task_elapsed_time, percentage_time, end_array[k].usStackHighWaterMark);
        }
    }
...
static int task_info(int argc, char **argv)
{
    ...
    if (task_info_args.processor_usage->count)
    {
        ESP_LOGI("TASK", "Display Processor Usage");
        esp_err_t ret = print_real_time_stats(10000 / portTICK_PERIOD_MS);
        ESP_LOGI("TASK", "%d, %s", ret, ret == ESP_OK ? "Real time stats obtained" : "Error getting stats");
    }
    else if (task_info_args.stack_usage->count)
    {
    }
...
static void register_task_info_command(void)
{
    ...
    task_info_args.stack_usage = arg_lit0("s", NULL, "Display Task Stack Usage");
    task_info_args.processor_usage = arg_lit0("p", NULL, "Display Task Processor Usage");
    task_info_args.end = arg_end(2);

    const esp_console_cmd_t cmd = {
        .command = "task",
        .help = "Task Info Commands",
        .hint = NULL,
        .func = &task_info,
